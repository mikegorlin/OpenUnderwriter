...
def _extract_distress_signals(
    ...
    is_financial = bool(
        sic_code and sic_code[:2] in ("60", "61", "62", "63", "64", "65", "66", "67")
    )
    ...
    _m_has_value = (
        m_result and m_result.value is not None and getattr(m_result, "status", "") != "SKIPPED"
    )
    if _m_has_value:
        try:
            ...
            _bv = None
        result["beneish_score"] = (
            f"{_bv:.3f}"
            if _bv is not None
            else (str(m_result.value) if m_result.value is not None else "N/A")
        )
        result["beneish_zone"] = (b.zone if b and b.zone else None) or m_result.evidence or "N/A"
        result["beneish_level"] = signal_to_display_level(
            m_result.status, m_result.threshold_level
        )
    else:
    ...
    beneish_sig = (
        m_result
        if m_result and m_result.do_context
        else safe_get_result(signal_results, "FIN.ACCT.earnings_manipulation")
    )
    result["beneish_do_context"] = (
        beneish_sig.do_context if beneish_sig and beneish_sig.do_context else ""
    )
    ...
    result["piotroski_do_context"] = (
        p_signal.do_context if p_signal and p_signal.do_context else ""
    )
    ...
    triggered_forensics = [s for s in forensic_signals if s.status == "TRIGGERED"]
...
def _extract_earnings_quality_signals(
    ...
    if quality_result and quality_result.value is not None:
        ...
        result["earnings_quality_level"] = signal_to_display_level(
            ...
            quality_result.threshold_level,
        )
...
def _extract_leverage_signals(
    ...
    if debt_structure and debt_structure.value is not None:
        ...
        result["leverage_level"] = signal_to_display_level(
            debt_structure.status,
            debt_structure.threshold_level,
        )
    ...
    result["debt_structure_do_context"] = (
        debt_struct_sig.do_context if debt_struct_sig and debt_struct_sig.do_context else ""
    )
    debt_cov_sig = safe_get_result(signal_results, "FIN.DEBT.coverage")
    result["debt_coverage_do_context"] = (
        debt_cov_sig.do_context if debt_cov_sig and debt_cov_sig.do_context else ""
    )
...
def _extract_tax_signals(
    ...
    result["etr_do_context"] = (
        etr_do_sig.do_context if etr_do_sig and etr_do_sig.do_context else ""
    )
...
def _extract_liquidity_signals(
    ...
    if liq_position and liq_position.value is not None:
        ...
        result["liquidity_level"] = signal_to_display_level(
            liq_position.status,
            liq_position.threshold_level,
        )
    if liq_trend and liq_trend.status == "TRIGGERED":
        _trend_ev = liq_trend.evidence or "Deteriorating"
        liq_parts.append(f"Trend: {humanize_check_evidence(_trend_ev)}")
    if cash_burn and cash_burn.status == "TRIGGERED":
        _burn_ev = cash_burn.evidence or "Alert"
        liq_parts.append(f"Cash Burn: {humanize_check_evidence(_burn_ev)}")
    ...
    result["liquidity_do_context"] = (
        liq_pos_sig.do_context if liq_pos_sig and liq_pos_sig.do_context else ""
    )
    cash_burn_sig = safe_get_result(signal_results, "FIN.LIQ.cash_burn")
    result["cash_burn_do_context"] = (
        cash_burn_sig.do_context if cash_burn_sig and cash_burn_sig.do_context else ""
    )
