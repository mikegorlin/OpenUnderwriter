...
def collect_extracted_field_keys(state: AnalysisState) -> set[str]:
    ...
    if extracted and hasattr(extracted, "financials") and extracted.financials:
    ...
    if extracted and extracted.market:
        ...
        if stk.returns_1y:
            keys.add("returns_1y")
        if stk.single_day_events is not None:
            keys.update(
                [
        ...
        if stk.beta_ratio:
            keys.add("beta_ratio")
        if stk.idiosyncratic_vol:
            keys.add("idiosyncratic_vol")
        if stk.max_drawdown_1y:
            keys.add("max_drawdown_1y")
        if stk.current_price:
            keys.add("current_price")
        if stk.pe_ratio:
            keys.add("pe_ratio")
        if stk.ev_ebitda:
            keys.add("ev_ebitda")
        if stk.peg_ratio:
            keys.add("peg_ratio")
        if stk.avg_daily_volume:
            keys.add("avg_daily_volume")
        if stk.volume_spike_count is not None:
            keys.add("volume_spike_count")
        ...
        if insider.exercise_sell_events:
            keys.add("insider_option_exercise_sell")
        if insider.timing_suspects:
            keys.add("insider_trading_pattern_deviation")
        ...
        an = mkt.analyst
        if an.analyst_count:
            keys.add("analyst_count")
        if an.recommendation_mean:
            keys.add("recommendation_mean")
        if an.recent_upgrades > 0 or an.recent_downgrades > 0:
            keys.add("estimate_revision_pattern")

        # Adverse events
        adv = mkt.adverse_events
        if adv.event_count is not None:
            keys.add("adverse_event_count")

        # Capital markets fields
        cm = mkt.capital_markets
        if cm.active_section_11_windows > 0:
            keys.add("event_ipo_exposure_score")

        # Stock drops analysis
        sd = mkt.stock_drops
        if sd.single_day_drops is not None:
            # Count single-day drops
            keys.add("single_day_drops_count")
            # Count unexplained drops
            _ = sum(1 for d in sd.single_day_drops if d.trigger_category == "unknown")
            keys.add("unexplained_drop_count")

    # ---- Alternative data field keys ----
    if state.alt_data:
        alt = state.alt_data
        if (
            hasattr(alt, "peer_sca")
            and alt.peer_sca
            and hasattr(alt.peer_sca, "contagion_risk")
            and alt.peer_sca.contagion_risk
        ):
            keys.add("peer_contagion_risk")
    ...
    if extracted and extracted.litigation:
        ...
        keys.update(
            [
                "employment_lit_count",
                "product_liability_count",
                "environmental_lit_count",
                "cyber_breach_count",
                "antitrust_count",
            ]
        )
